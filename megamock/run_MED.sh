...
run_anlys=${anlystrunk}/${run}
for mock in ${mocks[@]}; do
$mothur "#set.dir(output=${run_anlys}/MED/${mock}); \
set.logfile(name=${run_anlys}/MED/MED_${run}_log, append=T);\
seq.error(fasta=${run_anlys}/MED/${mock}/${mock}.fasta, count=${run_anlys}/MED/${mock}/${mock}_output.count, reference=~/expected_seqs/${mock}.fasta, aligned=F, ignorechimeras=T)\;
seq.error(fasta=${run_anlys}/MED/${mock}/${mock}.fasta, count=${run_anlys}/MED/${mock}/${mock}_output.count, reference=~/expected_seqs/${mock}.fasta, aligned=F, ignorechimeras=F)"&
done
