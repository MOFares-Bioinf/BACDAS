...
$usearch -fastx_truncate ${clustSpace}/${mock}/ref_${mock}.pcr.fasta  -trunclen 350 -fastaout ${clustSpace}/${mock}/ref_${mock}.trim.pcr.fasta

$mothur "#set.logfile(name=${clustSpace}/clust_log, append=T);\
align.seqs(fasta=${clustSpace}/${mock}/ref_${mock}.trim.pcr.fasta, reference=${trunk}/expected_seqs/silva_seed/silva.seed_v138_1.align );\
...
get.oturep(count=${clustSpace}/${mock}/ref_${mock}.trim.pcr.filter.unique.precluster.count_table, list=${clustSpace}/${mock}/ref_${mock}.trim.pcr.filter.unique.precluster.an.list, fasta=${clustSpace}/${mock}/ref_${mock}.trim.pcr.filter.unique.precluster.fasta, method=abundance, cutoff=0.03);" 
done
...
for mock in ${mocks[@]}; do 
echo ${mock}
cp ${clustSpace}/${mock}/ref_${mock}.trim.pcr.filter.unique.precluster.an.0.03.rep.fasta ${clustSpace}/reps/ref_${mock}.fasta
done
...
paste ${anlystrunk}/$Run/mockP_frequency > ${anlystrunk}/$Run/collective_frequency_$Run
...
paste ${anlystrunk}/$Run/mockP_clust_freq > ${anlystrunk}/$Run/collective_clust_freq_$Run
